...
static int parse(const int aOpt, const char *aArg)
{
	...
	switch (aOpt) {
	case 'k':
		strncpy_s(filenamePrivatekey, sizeof(filenamePrivatekey), aArg, PATH_MAX);
		break;
	case 'i':
		strncpy_s(filenameEncryptedInput, sizeof(filenameEncryptedInput), aArg, PATH_MAX);
		break;
	case 'o':
		strncpy_s(filenamePlaintextOutput, sizeof(filenamePlaintextOutput), aArg, PATH_MAX);
		break;
	case 'q':
		strncpy_s(keypassword, sizeof(keypassword), aArg, PATH_MAX);
		break;
	case 'Q':
...
int main(int argc, char **argv) {
	...
	if (genericOptHandler
        ...
        if (tpmVersion == 2.0){
            ...
            char command[1024] = "";
	    if( strnlen(filenamePlaintextOutput, PATH_MAX) > 0 ) {
    ...
    if (keypassword != NULL && strnlen(keypassword, PATH_MAX) > 0 ) {
      ...
      if( useEnvironment ) {
				...
				strncpy_s(keypassword, sizeof(keypassword), keypasswordEnv, PATH_MAX);
	  }
	  if( decodeHexPassword ) {
			...
			CATCH_TSS_ERROR( Tspi_Policy_SetSecret(hKeyPolicy, keypasswordMode, strnlen(keypassword, PATH_MAX), (BYTE *)keypassword) );			 
	  }
