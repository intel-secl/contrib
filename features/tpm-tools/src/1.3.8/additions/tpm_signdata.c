...
static int parse(const int aOpt, const char *aArg)
{
	...
	switch (aOpt) {
	case 'i':
		strncpy_s(filenameInput, sizeof(filenameInput), aArg, PATH_MAX);
		break;
	case 'k':
		strncpy_s(filenamePrivatekey, sizeof(filenamePrivatekey), aArg, PATH_MAX);
		break;
	case 'o':
		strncpy_s(filenameOutput, sizeof(filenameOutput), aArg, PATH_MAX);
		break;
	case 'q':
...
*/
	...
	if( strnlen(filenameOutput, PATH_MAX) > 0 ) {
