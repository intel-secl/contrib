#!/bin/bash
TBOOT_VERSION="1.9.7"
TBOOT="tboot-dist-${TBOOT_VERSION}"
...
generate_binary() {
  ...
  if yum_detect; then
    ...
    tboot_tgz=$(find . -name tboot-${TBOOT_VERSION}*-sources.tgz)
    mv "${tboot_tgz}" "tboot-${TBOOT_VERSION}.tar.gz"
    sudo -n rpmbuild -bb "rpm/tboot.spec" --define "_sourcedir $PWD" --define "_rpmdir $PWD" --nodeps
    sudo -n chown -R ${USER}:${USER} .
    tboot_rpm=$(find . -name tboot-${TBOOT_VERSION}*.rpm)
  ...
  # UBUNTU
    ...
    tboot_tgz=$(find . -name tboot-${TBOOT_VERSION}*-sources.tgz)
    tar fxz "${tboot_tgz}"
    mv debian "tboot-${TBOOT_VERSION}"
    (cd "tboot-${TBOOT_VERSION}" && sudo -n debuild -b -uc -us)
    sudo -n chown -R ${USER}:${USER} .
    tboot_deb=$(find . -name tboot-${TBOOT_VERSION}*.deb)
