...
%changelog
* Thu Aug 30 2018 Gang Wei <gang.wei@intel.com> - 1:1.9.7-1
...
* Tue Jul 11 2017 Ning Sun <ning.sun@intel.com> - 1:1.9.6-1
...
- Provide workaround for Intel PTT(Platform Trust Technology) & Linux PTT
  driver.
- Add new fields in Linux kernel header struct to accommodate Linux kernel new
  capabilities.
- Fix a pointer dereference regression in the tboot native Linux loader which
  manifests itself as a system reset.
- Fix the issue of overwriting tboot when the loaded elf kernel is located below
  tboot.
...
- Initiaize kernel header comdline buffer before copying kernel cmdline
  arguments to the buffer to avoid random data at end of the original cmdline
  contents.
- Move tpm_detect() to an earlier stage so as to get tpm interface initialized
  before checking TXT platform capabilities.
